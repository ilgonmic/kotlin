...
abstract class KtLightModifierList<out T : KtLightElement<KtModifierListOwner, PsiModifierListOwner>>(
    ...
    protected open fun nonSourceAnnotations(sourceAnnotations: List<PsiAnnotation>, forAnnotationType: Boolean): List<KtLightAbstractAnnotation> {
    ...
    private fun computeAnnotations(): List<KtLightAbstractAnnotation> {
        ...
        if (modifierListOwner is KtLightClassForSourceDeclaration && modifierListOwner.isAnnotationType) {
            ...
            val nonSourceAnnotations = nonSourceAnnotations(annotationsForEntries, forAnnotationType = true)
...
abstract class KtUltraLightModifierList<out T : KtLightElement<KtModifierListOwner, PsiModifierListOwner>>(
    ...
    override fun nonSourceAnnotations(sourceAnnotations: List<PsiAnnotation>, forAnnotationType: Boolean): List<KtLightAbstractAnnotation> {

        if (sourceAnnotations.isEmpty()) return emptyList()

        return sourceAnnotations.mapNotNull { sourceAnnotation ->
            sourceAnnotation.tryConvertAsTarget(support)
                ?: sourceAnnotation.tryConvertAsRetention(support, forAnnotationType = forAnnotationType)
