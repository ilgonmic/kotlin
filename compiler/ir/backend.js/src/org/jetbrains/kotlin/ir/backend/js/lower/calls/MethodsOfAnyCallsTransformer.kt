        ...
        nameToTransformer.run {
            put(Name.identifier("toString")) { call ->
                if (shouldReplaceToStringWithRuntimeCall(call)) {
                    if ((call as IrCall).isSuperToAny()) {
                        irCall(call, intrinsics.jsAnyToString, receiversAsFirstArguments = true)
                    } else {
                        irCall(call, intrinsics.jsToString, receiversAsFirstArguments = true)
                    }
                } else {
                    call
                }
            ...
            put(Name.identifier("hashCode")) { call ->
                if (call.symbol.owner.isFakeOverriddenFromAny()) {
                    if ((call as IrCall).isSuperToAny()) {
                        irCall(call, intrinsics.jsGetObjectHashCode, receiversAsFirstArguments = true)
                    } else {
                        irCall(call, intrinsics.jsHashCode, receiversAsFirstArguments = true)
                    }
                } else {
                    call
                }
