...
class NumberOperatorCallsTransformer(context: JsIrBackendContext) : CallsTransformer {
    ...
    private val memberToTransformer = MemberToTransformer().apply {
        ...
        irBuiltIns.booleanType.let {
            ...
            add(it, OperatorNames.AND) { call -> toBoolean(irCall(call, intrinsics.jsBitAnd, receiversAsFirstArguments = true)) }
            add(it, OperatorNames.OR) { call -> toBoolean(irCall(call, intrinsics.jsBitOr, receiversAsFirstArguments = true)) }
            add(it, OperatorNames.XOR) { call -> toBoolean(irCall(call, intrinsics.jsBitXor, receiversAsFirstArguments = true)) }
        ...
        return with(call.symbol.owner.valueParameters[0].type) {
            when {
                isByte() || isShort() || isInt() ->
                    irCall(call, intrinsics.jsNumberRangeToNumber, receiversAsFirstArguments = true)
                isLong() ->
                    irCall(call, intrinsics.jsNumberRangeToLong, receiversAsFirstArguments = true)
                else -> call
            }
        }
    }
    ...
    private fun irBinaryOp(
        ...
        val newCall = irCall(call, intrinsic, receiversAsFirstArguments = true)
        if (toInt32)
    ...
    private fun transformCrement(call: IrFunctionAccessExpression, correspondingBinaryOp: IrFunctionSymbol): IrExpression {
        val operation = irCall(call, correspondingBinaryOp, receiversAsFirstArguments = true).apply {
    ...
    private fun transformUnaryMinus(call: IrFunctionAccessExpression) =
        convertResultToPrimitiveType(
            irCall(call, intrinsics.jsUnaryMinus, receiversAsFirstArguments = true),
            call.type
        )
